  ...
  describe('Migration Service', () => {
    ...
    describe('forget', () => {
      it('should remove the most recent migration without calling the down function if no argument', async () => {
        ...
        migrationToForget.down = jest.fn().mockReturnValue(Promise.resolve()) as () => Promise<void>;
        try {
          await migrationService.applyMigration(migrationToForget);
          try {
            await migrationService.forgetMigration();
            expect(await migrationService.getAppliedMigrations()).not.toContain(migrationToForget);
            expect(await migrationService.getPendingMigrations()).toContain(migrationToForget);
      ...
      it('should forget a single migration by unique name', testForgetBy('name'));

      it('should refuse to forget a migration by non-unique name', testDontForgetAmbiguous('name', 'test_migration'));

      it('should forget a single migration by unique timestamp', testForgetBy('timestamp'));

      it('should refuse to forget a migration by non-unique timestamp', testDontForgetAmbiguous('timestamp', 1751567840547));
      ...
      function testForgetBy(k: 'name' | 'timestamp') {
        return async () => {
          ...
          const migrationToForget = migrations[0];
          const migrationNotToForget = migrations.find(m => m[k] !== migrationToForget[k])!;
          ...
          try {
            ...
            try {
              ...
              expect((await migrationService.getAppliedMigrations())
                .filter(m => m[k] === migrationToForget[k])
                .length
              ).toBe(1);
      ...
      function testDontForgetAmbiguous(k: 'name' | 'timestamp', v: string | number) {
        return async () => {
          const migrations = (await migrationService.getPendingMigrations())
            .filter(m => m[k] === v);
          expect(migrations.length).toBeGreaterThanOrEqual(2);
          const rollback = migrations.map(m => {
            const r = m.down;
            m.down = jest.fn().mockReturnValue(Promise.resolve()) as () => Promise<void>;
            return r;
          });
          try {
            await Promise.all(migrations.map(m => migrationService.applyMigration(m)));
